...
async function initDir<
	T extends { name: string; init?: () => void | Promise<void> },
>(dir: string): Promise<Discord.Collection<string, T>> {
	...
	for (const file of files) {
		const val = (await import(url.pathToFileURL(`${dir}${file}`).href)) as {
			default: T;
		};
		...
		try {
			if (command === undefined) {
				throw new BotError(
					...
					`\`${name}\` is not the name or alias of any command I have!`,
				);
			}
