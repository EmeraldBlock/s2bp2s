...
export function toEnglishList(array: string[]): string | undefined {
	switch (array.length) {
		case 0: {
			return undefined;
		}
...
export function fetchGuildMessageById(
	id: Discord.Snowflake,
	guild: Discord.Guild,
): Promise<Discord.Message> {
	const textChannelsCache = guild.channels.cache.filter(
		(channel): channel is Discord.TextChannel =>
			channel instanceof Discord.TextChannel,
	);
	return Promise.any(
		textChannelsCache.map((channel) => channel.messages.fetch(id)),
	);
}

export function makeListener<K extends keyof Discord.ClientEvents>(
	listenerArgs: Omit<Listener, "enable" | "disable">,
	event: K,
	listener: (...args: Discord.ClientEvents[K]) => void,
): Listener;
export function makeListener<S extends string | symbol>(
	listenerArgs: Omit<Listener, "enable" | "disable">,
	event: Exclude<S, keyof Discord.ClientEvents>,
