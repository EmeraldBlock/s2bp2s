	...
	try {
		images.set(
			...
			await Promise.all(
				new Array(LAYERS)
					.fill(undefined)
					.map((_, i) => loadImage(path.join(CACHE_DIR, `${name}-${i}.png`)))
			)
		);
	} catch {
...
const ORIGINS = [new Vector(0, 0), new Vector(1, 0), new Vector(1, 1), new Vector(0, 1)];
...
function sketch(blueprint: Blueprint) {
	...
	for (const entry of entrys) {
		...
		const corners = [0, bounds.dims.x]
			.flatMap((x) =>
				(x === 0 ? [0, bounds.dims.y] : [bounds.dims.y, 0]).map(
					(y) => new Vector(bounds.offset.x + x, bounds.offset.y + y)
				)
			)
		...
		for (const { pos, dir } of bounds.acceptors.concat(bounds.ejectors)) {
			...
			const w = v
				.sub(new Vector(0, 1 / 2).rotate(pr))
				.sub(ORIGINS[pr])
				.scale(SCALE)
				.add(ORIGINS[pr]);
			w.x += SCALE - 1 - SKEW * l;
...
export function render(blueprint: Blueprint) {
	...
	const canvas = createCanvas(
		(data.max.x - data.min.x) * TILE_SIZE * SCALE,
		(data.max.y - data.min.y) * TILE_SIZE * SCALE
	);
	...
	for (let l = 0; l < LAYERS; ++l) {
		...
		for (const step of data.layers[l]) {
			...
			if (!images.has(name)) {
				context.fillStyle =
					bounds === missing ? "#ff8080" : ["#808080", "#c0c0c0", "#e0e0e0"][l];
				...
				context.drawImage(
					...
					step.sx * IMAGE_SIZE,
					(step.sz * bounds.dims.y + step.sy) * IMAGE_SIZE,
					IMAGE_SIZE * (step.sw ?? 1),
					IMAGE_SIZE * (step.sh ?? 1),
					0,
					0,
					1,
					1
